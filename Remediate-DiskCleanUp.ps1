...
.DESCRIPTION
    Runs Disk Cleanup with predefined selections to free up disk space on the C: drive.
    Displays a WPF window to inform the user about the cleanup process.
    Uses SageRun registry settings to configure Disk Cleanup options.

.EXAMPLE
    .\Remediate-DiskCleanUp.ps1
...
$logFile = Join-Path $logFolder "Remediate-DiskCleanUp.log"
...
Write-Output "=== Disk Cleanup Remediation Started ==="
Write-Log    "Remediation started"

# Define cleanup type selections
$cleanupTypeSelection = @(
    ...
    'Device Driver Packages', 'Diagnostic Data Viewer database files', 'Downloaded Program Files', 'Feedback Hub Archive log files',
    'Internet Cache Files', 'Memory Dump Files', 'Old ChkDsk Files', 'Previous Installations', 'Previous Windows Installations',
    'Setup Log Files', 'System error memory dump files', 'System error minidump files', 'Temporary Files',
    'Temporary Setup Files', 'Temporary Sync Files', 'Update Cleanup', 'Upgrade Discarded Files', 
    'Windows Defender', 'Windows Error Reporting Files', 'Windows Error Reporting Archive Files',
    'Windows Error Reporting Queue Files', 'Windows Error Reporting System Archive Files',
    'Windows Error Reporting System Queue Files',
    'Windows ESD installation files'
)

# Function to set Disk Cleanup registry keys
function Set-SageRunRegistry {
    ...
    foreach ($selection in $Selections) {
        $regPath = Join-Path $basePath $selection
        try {
            if (Test-Path $regPath) {
                New-ItemProperty -Path $regPath -Name "StateFlags$RunId" -Value 2 -PropertyType DWord -Force | Out-Null
                Write-Log "Set $selection to be cleaned in SageRun $RunId"
                Write-Output "Set $selection to be cleaned in SageRun $RunId"
            }
            else {
                Write-Log "Warning: Registry path for '$selection' not found."
                Write-Output "Warning: Registry path for '$selection' not found."
            }
        }
        catch {
            Write-Log "Error setting registry for '$selection': $_"
            Write-Output "Error setting registry for '$selection': $_"
        }
    }
}


# Functions Free Disk Space
function Get-FreeSpaceGB {
    try {
        return [math]::Round((Get-PSDrive -Name C).Free / 1GB, 2)
        Write-Log "Free space retrieved successfully."
        Write-Output "Free space retrieved successfully."
    }
    catch {
        Write-Log "ERROR retrieving free space: $_"
        Write-Output "ERROR retrieving free space: $_"
        return $null
    }
}

# Functions to register notification app
...
# Function to set protocol action
function Set-Action {
...
if ($null -ne $FreeSpaceGB) {
    ...
    Write-Output "Current free space on C:: $FreeSpaceGB GB"
}

# Apply Disk Cleanup registry configuration
Set-SageRunRegistry -RunId $sageRunId -Selections $cleanupTypeSelection
...
Set-Action -Action_Name CleanDisk  -CmdBody $CleanCmd
...
$Message = "Your device does not have enough free space (30 GB). There is only $FreeSpaceGB GB left. You can free up space by clearing the Recycle Bin and the Downloads folder. Files in Downloads are not backed up anywhere. If you need to keep them, move them to OneDrive or Documents before cleaning."
...
#    <action activationType="protocol" arguments="$Action_CleanDisk"         content="Clean Disk now" />
#  </actions>
#"@
...
# Show toast
try { 
    Show-ToastXml -XmlString $Toast.OuterXml -AppID $AppID
    Write-Output "Toast notification process completed."
    Write-Log    "Toast notification process completed."
}
catch {
