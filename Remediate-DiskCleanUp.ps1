...
.DESCRIPTION
    This script performs disk cleanup by configuring and executing the built-in Disk Cleanup utility (cleanmgr.exe).
    It sets specific cleanup options via registry keys, including cleaning temporary files, recycle bin, previous installations, and more.
    The script also removes files from the user's Downloads folder and empties the Recycle Bin to free up additional space.
...
# Configure Disk Cleanup options
Write-Output "Configuring Disk Cleanup registry keys..."
Write-Log    "Configuring Disk Cleanup registry keys"
...
$cleanupTypeSelection = @(
    ...
    'Internet Cache Files', 'Memory Dump Files', 'Old ChkDsk Files', 'Previous Installations', 'Recycle Bin', 'Service Pack Cleanup',
    'Setup Log Files', 'System error memory dump files', 'System error minidump files', 'Temporary Files', 'Thumbnail Cache',
    'Update Cleanup', 'Upgrade Discarded Files', 'Windows Defender', 'Windows Error Reporting Archive Files',
...
foreach ($keyName in $cleanupTypeSelection) {
    try {
        New-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\VolumeCaches\$keyName" `
            -Name 'StateFlags0001' -Value 2 -PropertyType DWord -ErrorAction SilentlyContinue | Out-Null
    }
    catch {}
}

Write-Output "Disk Cleanup registry configuration completed."
Write-Log    "Registry keys set for Disk Cleanup"
